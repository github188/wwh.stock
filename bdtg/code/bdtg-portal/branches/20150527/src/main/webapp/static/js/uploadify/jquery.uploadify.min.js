...
SWFUpload.Console.writeLine = function(g) {
	...
	try {
		...
		tipsOp("Exception: " + h.name + " Message: " + h.message,'error');
	...
	var b = {
		...
		cancel : function(d, f) {
            ...
            var deleteFile = function(j, s, i, fid){
                fid &&c.getJSON(j.deleter + '&id=' + fid, function(json){
                    if (json.status == 1) {
                        var fileIds = c("#" + j.resText).val().toString();
                        var tmp = fileIds.split("|");
                        tmp.splice(i, 1);
                        fileIds = tmp.join(',');
                        c("#" + j.resText).val(fileIds);
					   ...
					   delete s.queueData.files[d];

					   s.queueData.filesQueued--;
					   s.queueData.filesSelected--;
					   s.queue.selector--;
					   s.queueData.uploadsSuccessful--;
						...
						if (e[0]) {
							if (e[0] == "*") {
								...
								for ( var m = 0; m < e.length; m++) {
									var fid = c("#" + e[m]).attr("fid");
									deleteFile(j, i,  c("#" + e[m]).index(), fid);
									i.cancelUpload(e[m]);
		...
		onDialogClose : function(d, f, g) {
			...
			if (c.inArray("onDialogClose", e.overrideEvents) < 0) {
				if (this.queueData.filesErrored > 0) {
					tipsOp(this.queueData.errorMsg,'error');
				}
			}
			if (e.onDialogClose) {
...
//						this.queueData.errorMsg += "\n选定文件数超过上传限制 ("+ f + ").";
		...
		onUploadComplete : function(f) {
			...
			if (c.inArray("onUploadComplete", g.overrideEvents) < 0) {
				c("#" + f.id).attr("fid", json.msg.fileid);
		...
		onUploadError : function(e, i, h) {
			...
			case SWFUpload.UPLOAD_ERROR.HTTP_ERROR:
				tipsOp('网络错误，请重试','error');
				g = "HTTP Error (" + h + ")";
				break;
			case SWFUpload.UPLOAD_ERROR.MISSING_UPLOAD_URL:
				tipsOp('上传网络地址错误，请重试','error');
				g = "Missing Upload URL";
				break;
			case SWFUpload.UPLOAD_ERROR.IO_ERROR:
				tipsOp('文件写入错误，请重试','error');
				g = "IO Error";
				break;
			case SWFUpload.UPLOAD_ERROR.SECURITY_ERROR:
				tipsOp('上传文件安全错误','error');
				g = "Security Error";
				break;
			case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
				tipsOp('上传文件数量超出限制，请重试','error');
				g = "Exceeds Upload Limit";
				break;
			case SWFUpload.UPLOAD_ERROR.UPLOAD_FAILED:
				tipsOp('上传文件安全错误，请重试','error');
				g = "Failed";
				break;
			case SWFUpload.UPLOAD_ERROR.SPECIFIED_FILE_ID_NOT_FOUND:
				break;
			case SWFUpload.UPLOAD_ERROR.FILE_VALIDATION_FAILED:
				tipsOp('上传文件验证错误，请重试','error');
				g = "Validation Error";
				break;
