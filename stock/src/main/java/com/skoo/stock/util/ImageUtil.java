...
public class ImageUtil {
     ...
     * @param path         保存图片地址
     */
    public static void abscutStream(BufferedImage bi, int x, int y, int destWidth,
                              int destHeight, String path) {
        try {
            Image img;
            ...
            if (srcWidth >= destWidth && srcHeight >= destHeight) {
                ...
                g.dispose();
                // 输出为文件
                saveImageUsingJPGWithQuality(tag, path, 0.6f);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static void saveImageUsingJPGWithQuality(BufferedImage image,
        ...
        if (image.getWidth() > 1242) {
            width = 1242;
        }

        Image newImage = image.getScaledInstance(width, width, Image.SCALE_DEFAULT);
        BufferedImage newBufferedImage = new BufferedImage(width, width, BufferedImage.TYPE_INT_RGB);
    ...
    public static void main(String[] args) {
        ...
        scale("c:/cas/sago.jpg", "c:/cas/sago2.jpg", 154, 166, 0, 0);
        convert("c:/cas/sago.jpg", "c:/cas/sago3.png");
        abscut("c:/cas/sago.jpg", "c:/cas/sago4.png", 50, 90, 500, 300);
