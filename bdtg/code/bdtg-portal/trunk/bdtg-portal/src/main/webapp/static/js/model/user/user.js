 ...
 * @returns {Boolean}
 */
function opConfirm(url,content) {
	
	var confirm = $.scojs_confirm({
		content : tipsType('您确定'+content+'吗？', 'warning'),
		action : function() {
			formSubmit(url, 'url');
		}
	});
 ...
 * 上架
 */
function grounding(url){
	var confirm = $.scojs_confirm({
		content : tipsType('您确定上架吗？', 'warning'),
		action : function() {
			formSubmit(url, 'url');
		}
	});
...
function undercarriage(url){
	var confirm = $.scojs_confirm({
		content : tipsType('您确定下架吗？', 'warning'),
		action : function() {
			formSubmit(url, 'url');
		}
	});
