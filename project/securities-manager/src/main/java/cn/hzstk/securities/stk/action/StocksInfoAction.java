package cn.hzstk.securities.stk.action;

import cn.hzstk.securities.common.service.StkCommQtz;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Controller;
import org.springframework.web.bind.annotation.RequestMapping;

import cn.hzstk.securities.common.action.MagicAction;
import cn.hzstk.securities.stk.domain.StocksInfo;
import cn.hzstk.securities.stk.service.StocksInfoService;
import org.springframework.web.bind.annotation.RequestMethod;

import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;


/**
*
* @description:StocksInfo action
* @author: autoCode
* @history:
*/
@Controller
@RequestMapping("/stk/stocks-info")
@SuppressWarnings("serial")
public class StocksInfoAction extends MagicAction<StocksInfo,StocksInfoService> {

    @Autowired
    private StkCommQtz stkCommQtz;

    /**
     * 生成记录.
     */
    @RequestMapping(value = "dataRefresh", method = RequestMethod.POST)
    public void dataRefresh(HttpServletRequest request,
            HttpServletResponse response) {
        try {
            StocksInfo o = bindEntity(request, entityClass);
            //stkCommQtz.execute();

            printJson(response, messageSuccuseWrap());
        } catch (Exception e) {
            logger.error("dataRefresh", e);
            printJson(response, messageFailureWrap("刷新失败！"));
        }
    }
}
