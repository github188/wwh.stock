...
import com.bm.wanma.adapter.MyCouponUsedAdapter;
 ...
 * 已使用
 */
public class CouponUsedFragment extends BaseFragment implements OnRefreshListener2<ScrollView>{
	...
	private MyCouponUsedAdapter maAdapter;
	private boolean isf;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if(isNetConnection()){
			GetDataPost.getInstance(getActivity()).getMyCouponList(handler, pkuserId, "2", String.valueOf(currentPage), pageNum);
		}
		return fragment;
	}
...
//			}
			if(isRefresh){
				if (rawdata.size()!=beans.size()&&isf&&CouponchangeSize!=null) {
					CouponchangeSize.coupondeprecated();
				}
					beans.clear();
...
//			}
			...
			if(beans.size()>0){
				if(maAdapter == null){
					maAdapter = new MyCouponUsedAdapter(getActivity(), beans);
