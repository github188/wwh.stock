...
import cn.hzstk.securities.member.domain.*;
import cn.hzstk.securities.member.mapper.AuthCompanyMapper;
import cn.hzstk.securities.member.mapper.AuthRealnameMapper;
import cn.hzstk.securities.member.mapper.AuthSpaceMapper;
import cn.hzstk.securities.member.service.*;
...
public class MemberAction extends BaseMagicAction {
     ...
     *
     */
    @RequestMapping(value="mobile_only")
    public void mobile_only(HttpServletRequest request,HttpServletResponse rep,String mobile) {
        try {
            Map<String, String> param = new HashMap<String, String>();
            List<AuthSpace> authSpaces = new ArrayList<>();
            authSpaces = authSpaceService.querybymobile(mobile);
            if(authSpaces.size()==0){
                param.put("vars", "Y");
            }else{
                param.put("vars", "N");
     ...
     *
     */
    @RequestMapping(value="email_only")
    public void email_only(HttpServletRequest request,HttpServletResponse rep,String email) {
        try {
            Map<String, String> param = new HashMap<String, String>();
            List<AuthSpace> authSpaces = new ArrayList<>();
            authSpaces = authSpaceService.querybyemail(email);
            if(authSpaces.size()==0){
                param.put("vars", "Y");
            }else{
                param.put("vars", "N");
