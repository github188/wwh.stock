...
import com.bm.wanma.adapter.MyBillsDetailsAdapter;
 ...
 * 账单明细
 */
public class MyBillDetailsActivity extends BaseActivity implements OnRefreshListener2<ScrollView>{
	...
	private ArrayList<BillDetailsBean> data,rawdata;
	private MyBillsDetailsAdapter maAdapter;
	private String moth;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mPageName = "MyBillDetailsActivity";
		moth = getIntent().getStringExtra("datatime");
		initView();
	}
	
	private void initView(){

		tv_bill_title = (TextView) findViewById(R.id.bill_title);
		tv_bill_title.setText(TimeUtil.parseDate(moth,
				"yyyy-MM", "M月")+"账单明细");
		...
		currentPage = 1;
		isRefresh = true;
		data = new ArrayList<BillDetailsBean>();
		rawdata = new ArrayList<BillDetailsBean>();
		if(isNetConnection()){
			GetDataPost.getInstance(this).getMyMothBillList(handler, pkuserId, moth,String.valueOf(currentPage), pageNum);
		}
	}
...
protected void onPause() {
	...
	public void onSuccess(String sign, Bundle bundle) {
		if (bundle != null) {
			...
			rawdata = (ArrayList<BillDetailsBean>) bundle.getSerializable(Protocol.DATA);
			if(rawdata.size()<20){
			...
			if(data.size()>0){
				if(maAdapter == null){
					maAdapter = new MyBillsDetailsAdapter(getActivity(), data);
