...
import com.ijustyce.fastandroiddev3.databinding.ListFragmentView;
...
public abstract class BaseListFragment<Bind extends ViewDataBinding, Bean, 
        Model extends IResponseData<Bean>> extends BaseFragment<Bind> {
    ...
    final void initViewAndData() {
        ...
        iDataInterface = new IDataInterface<Bean, Model>(context, bindingInfo) {
            ...
            public void dataLoadFailed(Throwable throwable) {
                ...
                BaseListFragment.this.dataLoadFailed(throwable);
            }

            @Override
            public boolean onDataGet(Model data) {
                if (activity == null || contentView == null) return false;
                return BaseListFragment.this.onDataGet(data);
            }

            @Override
            public void onDataShow(ArrayList<Bean> data) {
                if (activity == null || contentView == null) return;
                BaseListFragment.this.onDataShow(data);
            }

            @Override
            public void afterBindToView(Model model) {
                BaseListFragment.this.afterBindToView(model);
            }
        };
    ...
    public View getEmptyView(){
        ...
        if (contentView instanceof ListFragmentView && ((ListFragmentView) contentView).noData != null) {
            ViewStub viewStub = ((ListFragmentView) contentView).noData.getViewStub();
            synchronized (this) {
                if (emptyView == null) {
    ...
    public IRecyclerView getRecyclerView() {
        if (contentView instanceof ListFragmentView) {
