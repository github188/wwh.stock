...
import cn.hzstk.securities.sys.service.PermissionService;
...
public class PermissionAction extends MagicAction<Permission, PermissionService> {
    ...
    public void saveRolePermissions(HttpServletRequest request,HttpServletResponse response,@RequestParam(value = "roleId")String roleId,@RequestParam(value = "permissions")String permissions) {
        entityService.saveRolePermissions(Long.valueOf(roleId),permissions,this.getCurrentUser(request).id);
