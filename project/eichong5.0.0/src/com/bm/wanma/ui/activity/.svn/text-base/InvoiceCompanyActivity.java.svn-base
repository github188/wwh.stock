...
import com.bm.wanma.popup.CustomTipInvoicePopuWindow;
 ...
 * 填写发票详情  公司
 */
public class InvoiceCompanyActivity extends BaseActivity implements OnClickListener {
	...
	private ContainsEmojiEditText et_company_title,et_shibiehao,et_company_addr,et_company_phone;
	private ContainsEmojiEditText et_company_bank,et_company_zhanghu,et_name,et_phone,et_addr;
	...
	private String companyName,taxpayerNumbe,companyAddr,companyPhone,bankName,bankAccount;
	private String recipients,recipientsPhone,recipientsAddr,orderId;
	//alipay
	...
	private CustomTipInvoicePopuWindow popupWindow;
	@Override
	...
	private void initpopuwindow() {
		if (popupWindow == null) {
			popupWindow = new CustomTipInvoicePopuWindow(this);
			...
			popupWindow.showAtLocation(findViewById(R.id.rl_main),
	...
	private void initView(){
		...
		et_shibiehao = (ContainsEmojiEditText) findViewById(R.id.invoice_company_et_shibiehao);
		et_company_addr = (ContainsEmojiEditText) findViewById(R.id.invoice_company_et_com_addr);
		et_company_phone = (ContainsEmojiEditText) findViewById(R.id.invoice_company_et_com_phone);
		et_company_bank = (ContainsEmojiEditText) findViewById(R.id.invoice_company_et_bank);
		et_company_zhanghu = (ContainsEmojiEditText) findViewById(R.id.invoice_company_et_zhanghu);
	...
	private void initData(){
		...
		try {
			infoList = finalDb.findAllByWhere(InvoiceInfoDetailBean.class, "User_ID=\'"+uId +"\' and Type=1");
			if(infoList != null && infoList.size()>0){
				et_company_title.setText(infoList.get(0).getCompanyName());
				et_shibiehao.setText(infoList.get(0).getTaxNum());
				et_company_addr.setText(infoList.get(0).getCompanyAddr());
				et_company_phone.setText(infoList.get(0).getCompanyPhone());
				et_company_bank.setText(infoList.get(0).getCompanyBank());
				et_company_zhanghu.setText(infoList.get(0).getBank());
	...
	private void handleAddr(){
		...
		try {
			...
			new Thread(new Runnable() {
				@Override
				public void run() {
					finalDb.creatTable(InvoiceCompanyActivity.this,"t_m_area");
				}
			}).start();
		}
	...
	private void handleCommitInvoice(){
		...
		taxpayerNumbe = et_shibiehao.getText().toString();
		companyAddr = et_company_addr.getText().toString();
		companyPhone = et_company_phone.getText().toString();
		bankName = et_company_bank.getText().toString();
		bankAccount = et_company_zhanghu.getText().toString();
		...
		if(TextUtils.isEmpty(taxpayerNumbe)){
			showToast("请填写纳税人识别号");
			return;
		}
		...
		if(!RegularExpressionUtil.isPhone(companyPhone) && !RegularExpressionUtil.isMobilephone(companyPhone)){
			showToast("公司电话格式有误");
			return;
		}
		...
		showPD("提交开票请求");
		GetDataPost.getInstance(this).commitInvoice(handler, uId, "1", invoiceContent, companyName, pmoney,
				taxpayerNumbe, companyAddr, companyPhone, bankName, bankAccount, payment, recipients, recipientsPhone, 
	...
	private void saveDB(){
		...
		bean.setTaxNum(taxpayerNumbe);
		bean.setCompanyAddr(companyAddr);
		bean.setCompanyPhone(companyPhone);
		bean.setCompanyBank(bankName);
		bean.setBank(bankAccount);
		...
		try {
			if(infoList != null && infoList.size()>0){
				finalDb.update(bean,"User_ID=\'"+uId +"\' and Type=1");
			}else {
				finalDb.save(bean);
			}
		} catch (Exception e) {
	...
	//选择省市区
		private void handleSelectAddress(){
			...
			final MyAlertDialog dialog1 = new MyAlertDialog(
					InvoiceCompanyActivity.this).builder().setTitle("请选择省市区")
					.setView(view)
		...
		private View dialog() {
			View contentView = LayoutInflater.from(InvoiceCompanyActivity.this).inflate(
				...
				case SDK_PAY_FLAG: {
					...
					if (TextUtils.equals(resultStatus, "9000")) {
						...
						if (TextUtils.equals(resultStatus, "8000")) {
							Toast.makeText(InvoiceCompanyActivity.this, "支付结果确认中",
		...
		private void callaliPay() {
			...
			Runnable payRunnable = new Runnable() {
				...
				public void run() {
					// 构造PayTask 对象
					PayTask alipay = new PayTask(InvoiceCompanyActivity.this);
					// 调用支付接口，获取支付结果
