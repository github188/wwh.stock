...
$(function(){
	...
	//		setTimeout('$.dream.closetip();', 500 * 1);
	//	}
	//});
...
function addTab(subtitle,menuid,icon){
	if(!$('#tabs').tabs('exists',subtitle)){
		if (menuid.indexOf("?") != -1) {
			menuid = menuid + "&time=" + Math.random();
		} else {
			menuid = menuid + "?time=" + Math.random();
		}
		...
		$('#tabs').tabs('addIframeTab', {
			tab: {title: subtitle, closable: true, iconCls: icon},
			iframe: {src: Man.context + menuid,delay: 0}
		});
	}else{
...
{
	if (src.indexOf("?") != -1) {
		src = src + "&time=" + Math.random();
	} else {
		src = src + "?time=" + Math.random();
	}

	var scontent = '<iframe ame="infoframe" id="infoframe" scrolling="auto" frameborder="0"  src="'+Man.context+src+'" style="width:100%;height:100%;"></iframe>';
	return scontent;
}
