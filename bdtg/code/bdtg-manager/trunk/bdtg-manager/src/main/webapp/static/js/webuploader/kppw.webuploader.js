...
;(function($, w) {
	...
	function Upload($fileInput, options,seropts) {
		...
		var contextPath = "";
		this.baseUrl = w.baseurl !== undefined?w.baseurl:contextPath;
	...
	$.extend(Upload.prototype, {
		...
		absolutePath:function(){
			if(this.baseUrl != undefined){
				this.options.server = this.baseUrl + '/' + this.options.server + "&ossType=" + this.options.ossType;
		...
		,delFile:function(){
			...
			$(selecter).on('click','.webuploader-pick-file-close',function(){
				...
				if(fid > 0){
					$.post(self.options.delUrl,{"id":fid,"url":furl},function(json){
						if(json.status == '1'){
	...
	$.fn[pluginName].defaults = {
		...
		/** 
	     ...
	     server			: 'api/file/fileupload?me=up',// 文件接收服务端。
	     /**
	      ...
	      */
	     uploadEvents   : {},  //上传事件
		 delFile		: false,//文件删除是否开启，开启后上传文件显示列表有删除文件按钮，默认开启
