 ...
 * 过期
 */
public class CouponDeprecatedFragment extends BaseFragment implements OnRefreshListener2<ScrollView>{
	private boolean isf;
	...
	private MyCouponDeprecateAdapter maAdapter;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if(isNetConnection()){
			GetDataPost.getInstance(getActivity()).getMyCouponList(handler, pkuserId, "3", String.valueOf(currentPage), pageNum);
		}
		return fragment;
	}
...
//			}
			...
			if(beans.size()>0){
				if(maAdapter == null){
					maAdapter = new MyCouponDeprecateAdapter(getActivity(), beans);
