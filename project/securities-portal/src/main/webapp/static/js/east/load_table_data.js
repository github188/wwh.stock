...
Object.extend(LoadTable.prototype, {
    ...
    display: function () {
        ...
        if (_d && _d.length && _d[0].stats == undefined) {
            ...
            for (var i = 0; i < _d.length; i++) {
                ...
                if (data.SZZBL != "-" && data.GQDJR == "-" && data.CQCXR == "-") {
                    s.push(data.Code);
                } else if (document.getElementById("code_list") != null) {
                    s.push(data.join("|"));
                }
            }
            if (s.length > 0 && s[0].indexOf("|") == -1) {
                _t.tools.loadJs(portalurl+"stockeast/selfsave?yjfpList=" + s.join(","), _t.options.charset, function(){});
            } else if (document.getElementById("code_list") != null) {
                document.getElementById("code_list").value = s.join(",");
            }
        } else {
            _t.showNoData()
        }
    ...
    parperUrl: function () {
        ...
        _url = _url.replace("{param}", param.replace("全部股票", "%C8%AB%B2%BF%B9%C9%C6%B1")
            .replace("沪市", "%BB%A6%CA%D0").replace("深主板", "%C9%EE%D6%F7%B0%E5")
            .replace("中小板", "%D6%D0%D0%A1%B0%E5").replace("创业板", "%B4%B4%D2%B5%B0%E5"));
        return _url
    },
    update: function () {
        var _t = this;
